...
def evaluate_on_env(model, device, context_len, env, rtg_target, rtg_scale,
                    num_eval_ep=10, max_test_ep_len=10,
    ...
    with torch.no_grad():
        ...
        for i in range(num_eval_ep):
            print("~~~~~~~~~eval_round~"+str(i)+"~~~~~~~~~")
            # zeros place holders
            actions = torch.zeros((eval_batch_size, max_test_ep_len, act_dim),
                                dtype=torch.float32, device=device)
            states = torch.zeros((eval_batch_size, max_test_ep_len, state_dim[0], state_dim[1], state_dim[2]), #img 3 * 800 * 800
                                dtype=torch.float32, device=device)
            ...
            # random_action_index
            init_action = (0, 0.3)
            running_state = env.reset(init_action)
            running_reward = 0
