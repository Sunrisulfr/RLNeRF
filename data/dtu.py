...
class MVSDatasetDTU(Dataset):
    ...
    def build_metas(self):
        ...
        #scan * 49 * light_idx  == 88 * 49 * 7 = 30184 (train)
    ...
    def build_proj_mats(self):
        ...
        self.proj_mats= np.stack(proj_mats)
    ...
    def __getitem__(self, idx):
        ...
        if self.split=='train':
            # ids = torch.randperm(self.n_views)[:self.n_views] #n_views
            # ids = torch.randint(5, (5,))[:self.n_views]
            ids = torch.randint(self.n_views, (self.n_views,))[:self.n_views]
            view_ids = [src_views[i] for i in ids] + [target_view]
        else:
            view_ids = [src_views[i] for i in range(self.n_views)] + [target_view] #n_views
