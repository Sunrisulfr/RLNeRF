...
class MVSNet(nn.Module):
    def __init__(self,
        ...
        self.cost_reg_2 = CostRegNet(32+3*self.n_views, norm_act) #n_views

    def build_volume_costvar(self, feats, proj_mats, depth_values, pad=0):
        # feats: (B, V, C, H, W) C = 32, V = 3
        # proj_mats: (B, V, 3, 4)
    ...
    def build_volume_costvar_img(self, imgs, feats, proj_mats, depth_values, pad=0):
        ...
        img_feat = torch.empty((B, 3 * self.n_views+ 32, D, *ref_feats.shape[-2:]), device=feats.device, dtype=torch.float)  #n_views
        ...
        # print(volume_sq_sum.shape) #[1, 32, 128, 176, 208]
        # exit()
        ...
        img_feat[:, -32:] = volume_sq_sum * count - (volume_sum * count) ** 2 #[1,41,128,176,208]
        del volume_sq_sum, volume_sum, count
    ...
    def forward(self, imgs, proj_mats, near_far, pad=0,  return_color=False, lindisp=False):
        ...
        assert len(imgs) == len(proj_mats), "Different number of images and projection matrices"
        ...
        for src_fea, src_proj in zip(src_features, src_projs):
            # warpped features
            warped_volume = homo_warp(src_fea, src_proj, ref_proj, depth_values)
            if self.training:
                volume_sum = volume_sum + warped_volume
                volume_sq_sum = volume_sq_sum + warped_volume ** 2
            else:
        ...
        print(feats_l.shape, feats_l.shape, proj_mats.shape,"!!!")
        volume_feat, in_masks = self.build_volume_costvar_img(imgs, feats_l, proj_mats, depth_values, pad=pad)
        print("~~~~~~~~~~~~~")
        print(imgs.shape, "~", volume_feat.shape, "~", feats_l.shape, "~", depth_values.shape)
        if return_color:
        ...
        print(volume_feat.shape, "~", feats_l.shape, "~", depth_values.shape)
...
class RMVSNet(nn.Module):
    def __init__(self,
        ...
        self.cost_reg_2 = CostRegNet(32+9, norm_act) #n_views
    ...
    def build_volume_costvar_img(self, imgs, feats, proj_mats, depth_values, pad=0):
        ...
        img_feat = torch.empty((B, 9 + 32, D, *ref_feats.shape[-2:]), device=feats.device, dtype=torch.float)  #n_views
        imgs = F.interpolate(imgs.view(B * V, *imgs.shape[2:]), (H, W), mode='bilinear', align_corners=False).view(B, V,-1,H,W).permute(1, 0, 2, 3, 4)
        img_feat[:, :3, :, pad:H + pad, pad:W + pad] = imgs[0].unsqueeze(2).expand(-1, -1, D, -1, -1)
        # print(ref_feats.shape) # (B, C, h, w)
        ref_volume = ref_feats.unsqueeze(2).repeat(1, 1, D, 1, 1)  # (B, C, D, h, w)
    ...
    def forward(self, imgs, proj_mats, near_far, pad=0,  return_color=False, lindisp=False):
        ...
        B, V, _, H, W = imgs.shape #n_views=V
        # print(imgs.shape,"!!!")
        imgs = imgs.reshape(B * V, 3, H, W)
        feats = self.feature(imgs)  # (B*V, 8, H, W), (B*V, 16, H//2, W//2), (B*V, 32, H//4, W//4)
        # print(feats.shape)
